  ...
  return (
    <div className="admin-login-page">
      <div className="admin-login-card">
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, margin: '16px 0', color: 'var(--admin-text-muted)', fontSize: 12 }}>
          <div style={{ flex: 1, height: 1, background: 'var(--admin-border)' }} />
          <span>or</span>
          <div style={{ flex: 1, height: 1, background: 'var(--admin-border)' }} />
        </div>

        <button className="admin-google-btn" onClick={handleGoogleLogin} disabled={loading}>
        ...
        {(error || stateError) && (
          <div className="admin-login-error">{error || stateError}</div>
        )}
      </div>
    </div>
  );
